---
title: 'Class 4: Hierarchical generalised linear models'
...
Here $logit(p_i)$ is the link function, and equal to $\log \left( \frac{p_i}{1-p_i} \right)$ and transforms the bounded probabilities into an unbounded space

- If we have non-binary data we just change the likelihood:
$$y_i \sim Bin(N_i, p_i), logit(p_i) = \alpha + \beta x_i$$
...
$$y_i \sim Bin(N_i, p_i), logit(p_i) = \alpha_{\mbox{altitude}_i} + \beta_{\mbox{altitude}_i} x_i$$
