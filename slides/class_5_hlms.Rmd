...
\small
- The neat way to write this model is to use the double indexing:
```
...
- In general a hierarchical model is created whenever we put prior distributions on parameters that depend on other parameters
...
model {
  ...
  sigma ~ dt(0, 2, 1)T(0,) # Note new t prior
}
'
```
...
- The second is called the Widely Applicable Information Criterion (WAIC) which is calculated as:
