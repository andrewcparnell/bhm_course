...
```
log_earn[i] ~ dnorm(alpha1 * ethnicity_1 + alpha2 * ethnicity_2 + ...
                    + beta * height[i],
                          sigma^-2)
```
...
for(i in 1:4) {
  hist(pars$beta[,i], breaks = 30, main = eth_names[i], xlim = range(pars$beta), xlab = 'Slope value', freq = FALSE) 
  lines(density(pars_old$beta[,i]))
}
par(mfrow=c(1,1))
```
