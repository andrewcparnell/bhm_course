...
- Some possible choices:
    ...
    - Add race in as a covariate, this will lead to a _varying intercepts_ model
    - Add race in as an interaction, this will lead to a _varying slopes_ model
    
- An example of the varying slopes model is:

$$y_i \sim N(\alpha + \beta_{\mbox{race}_i} (x_i - \bar{x}), \sigma^2) $$
...
- A better choice is to put a prior distribution on the alpha and beta values that _ties them together_ and add some extra parameters. For example, $\alpha_{\mbox{race}_i} \sim N(\mu_\alpha, \sigma_\alpha^2)$, and $\beta_{\mbox{race}_i} \sim N(\mu_\beta, \sigma_\beta^2)$

- Under this model we can _borrow strength_ between the different races, and put extra prior distributions on the new parameters $\mu_\alpha, \mu_\beta, \sigma_\alpha, \sigma_\beta$
...
- The parameters $\mu_\alpha$ and $\mu_\beta$ have useful interpretations: they are the overall mean intercept and the overall mean slope for all observations removing the effect of race

- The parameters $\sigma_\alpha$, and $\sigma_\beta$ have useful interpretations too: they are the variability in the intercept and slope between race
...
par(mfrow=c(2,2))
race_names = c('Whites','Blacks','Hispanics','Others')
for(i in 1:4) {
  curr_dat = subset(dat, dat$race == i)
  plot(curr_dat$height_cm, log(curr_dat$earnings), main = race_names[i], ylab = 'log(earnings)', xlab = 'Height (cm)')
...
pars = jags_run$BUGSoutput$sims.list
for(i in 1:4) {
  hist(pars$beta[,i], breaks = 30, main = race_names[i], xlim = range(pars$beta), xlab = 'Slope value')  
}
par(mfrow=c(1,1))
```
...
- By contrast the standard deviations of alpha and beta, $\sigma_\alpha$ and $\sigma_\beta$ are estimated based on just the four different race groups
