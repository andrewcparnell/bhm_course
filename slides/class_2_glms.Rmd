...
- The rate ($\lambda$) of the Poisson distribution has to be positive, so taking the log of it changes it range to $(-\infty,\infty)$ as before. The inverse-link ($\exp$) turns the unrestricted ranges into something that must be positive
...
- The above allows us to simulate from the model, as given some parameters $\theta$ and some covariates $x$ we can use the probability distribution to get simulated data
...
- We can extent LMs and GLMs to have multiple covariates if we want, e.g.
```{r, eval=FALSE}
...
- Going back to the earnings data, suppose we want to fit a model to predict log earnings based on sex and whether respondent is Hispanic or not
- The model is:
$$\log(\mbox{earnings}) \sim N(\alpha + \beta_1 \mbox{height} + \beta_2 \mbox{hispanic}, \sigma^2)$$
...
- We can plot, for example the fitted mean
```{r}
...
plot(dat$height_cm, log(dat$earnings))
lines(dat$height_cm, alpha_mean + beta1_mean * dat$height_cm)
lines(dat$height_cm, alpha_mean + beta1_mean * dat$height_cm, col = 'red')
lines(dat$height_cm, alpha_mean + beta1_mean * dat$height_cm + beta2_mean, col = 'blue')
```
...
- There are three key parts of the algorithm that affect how good the posterior samples are:
    ...
    2. The way you choose your new parameter values. If they are too close to the previous values the MCMC might move too slowly so you might need to _thin_ the samples out by taking e.g. every 5th or 10th iteration
...
y_sim = rnorm(nrow(dat), 
              post[1,'alpha'] + 
                ...
                post[1,'beta2'] * (dat$hispanic-1), 
              sd = post[1, 'sigma'])
plot(log(dat$earnings), y_sim)
```
...
```
```{r}
plot(log(dat$earnings), apply(pars,2,'mean'))
```
...
- Just like the linear regression example, we can create posterior preditive distributions for the binary data from the binomial distribution
