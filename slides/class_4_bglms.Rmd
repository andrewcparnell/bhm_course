...
- In general, we use a _generalised linear model_ (GLM) when the normal distribution is not longer appropriate for the data

- This probability distribution should match the type of data (e.g. count data, binary, etc) and will have its own parameters

- We pften have to transform the parameters if we still want to use a linear regression type relationship with a covariate. The transformation is called a _link function_

- For a Bayesian model the probability distribution allows us to calculate a likelihood and to be able to get some priors for the unknown parameters
