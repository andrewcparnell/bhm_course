...
- The key to choosing a prior distribution is to put something in you believe represents the reasonable range of values, or comes from a related study in the literature
- Again, use an _informative prior_ if possible

Note: the shape of the distribution is usually completely unimportant when choosing the likelihood!

## Stan vs JAGS

We will be using two different software tools to calculate posterior distributions which represent the state of the art for user-friendly, research quality Bayesian statistics.
...
- In more complex, you need the prior to constrain some of the parameters so that the model will fit. These are known as _regularisation_ priors
...
1192 observations on earnings (in USD) and various measurements about education, height, etc
...
- Are there different rates of change for different groups (e.g. men vs women)?


## Example 3: Swiss Willow Tit data

3 replicate measurements on whether Swiss Willow Tits were found with a covariates on forest cover and elevation
\tiny
```{r, echo=FALSE}
