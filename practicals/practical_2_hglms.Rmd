---
title: "Practical 2 - Fitting hierarchical GLMs"
...
In practical 2 we are going to:
...
Let's create a much richer plot to that created in Class 3. The original plot we had was created with:
...
for(i in 1:4) {
  ...
  plot(curr_dat$height_cm, log(curr_dat$earn), main = eth_names[i], ylab = 'log(earnings)', xlab = 'Height (cm)')
  for(j in 1:20) {
    lines(dat$height_cm, pars$alpha[j,i] + pars$beta[j,i]*(dat$height_cm - mean (dat$height_cm)), col = i)  
  }
}
```
...
1. Another common plot is that of the residuals vs the fitted values. Creat a nice plot of this too. Interpret the plot.
...
In Class 4 we met lots of different hierarchical GLM models. The code for all of these models is in the Class 4 `.Rmd` file. Rather than reproduce all the code here, I suggest working through these exercises:

***
**Exercise 3**  

1. Go through each model in turn in the Class 4 notes. Make sure it will run and check the convergence. Run the model for longer if you don't think it has converged.
...
At the beginning of Class 3 we created a prior predictive distribution for the earnings data. To do this we went through the following steps:

1. We hypothesised a model and drew a DAG
...
Let's now create a prior predictive distribution for the over-dispersed Poisson example of Class 4. The JAGS code for a starting model might be:
```{r}
jags_code = '
model{
  # Likelihood
...
First load in the data and create the sum of `rep.1`, `rep.2`, and `rep.3` ignoring the missing values, as discussed in Class 4:
...
- We haven't covered cross-validation in this code at all. You might like to try and code up a cross-validation version of some of these models. You can copy the example from Class 3
- Try and create posterior predictive distributions for all of the models fitted in Class 4. 