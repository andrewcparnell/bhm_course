...
__Mixed effects models__: A Frequentist model containing both _fixed effects_ (parameters which do not bottow strength across groups), and _random effects_ (parameters which do borrow strength across groups). In a hierarchical models the aim is always to use the random effects approach.
...
__Latent parmameters__: A parameter which is hidden in the data and can be estimated by using a statistical model (such as a hierarchical model)
